...
프롬프트 분석과 생성을 담당하는 기본 모델은 `gemma-4-31b-it`입니다. 배포 환경에서 Forge가 다른 모델 ID를 제공하는 경우 `BUILT_IN_LLM_MODEL` 환경 변수로 재정의할 수 있습니다. 성숙한 로맨스나 관능적 분위기는 성인 간의 비노골적 표현으로만 보완하며, 노골적인 성행위·포르노성 묘사·미성년자·비동의 성적 상황은 허용하지 않습니다.
