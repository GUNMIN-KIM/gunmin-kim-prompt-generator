...
프롬프트 분석과 생성을 담당하는 기본 모델은 `gemma-4-31b-it`입니다. 배포 환경에서 Forge가 다른 모델 ID를 제공하는 경우 `BUILT_IN_LLM_MODEL` 환경 변수로 재정의할 수 있습니다. 최종 출력은 로맨스·로맨틱 코미디·감정선·관계성·성적 분위기를 임의로 추가하지 않고, 피사체 동선·구도·렌즈·카메라·조명·재질·가림 관계·컷 전환·시간적 연속성 같은 관찰 가능한 시각 연출에 집중합니다. 노골적인 성행위·포르노성 묘사·미성년자·비동의 성적 상황은 허용하지 않습니다.
