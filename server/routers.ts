  ...
  prompt: router({
    generate: publicProcedure.input(promptInput).mutation(async ({ input }) => {
      ...
      const response = await invokeLLM({
        ...
        messages: [
          { role: "system", content: "당신은 Google Gemma 4 기반의 영상 디렉터이자 비디오 생성 프롬프트 에디터입니다. 한국어로 답하고 추상적인 표현보다 피사체의 동선·속도·카메라 움직임·렌즈·빛·전환·시간적 연속성을 구체적으로 보완합니다. 성인 간 친밀감이나 로맨스 요청은 분위기만 반복하지 말고, 상호 동의가 전제된 손을 잡기·다가가기·포옹·키스·시선·호흡·카메라 거리·페이드아웃 등 화면에 보이는 비노골적 행동을 2~4단계 시간 순서로 명시합니다. 참조 이미지 또는 영상 분석이 있으면 보이는 시각적 특징만 활용하며 실제 인물의 신원을 식별하거나 추측하지 않습니다. 노골적인 성행위·성기·나체 중심 묘사·포르노성 지시, 미성년자, 비동의 또는 착취적 성적 상황은 생성하지 말고 비노골적인 대안으로 바꿉니다. 원본 영상 편집과 VFX 덧방에서는 수정 범위를 엄격히 분리하고, 사용자가 지정하지 않은 인물·카메라·프레이밍·배경을 보존합니다. JSON만 출력합니다." },
          { role: "user", content },
        ],
