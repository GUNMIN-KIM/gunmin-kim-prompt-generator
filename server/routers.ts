...
export const appRouter = router({
  ...
  prompt: router({
    generate: publicProcedure.input(promptInput).mutation(async ({ input }) => {
      ...
      const content: Array<{ type: "text"; text: string } | { type: "image_url"; image_url: { url: string; detail: "auto" } }> = [
        { type: "text", text: `${detailLines}\n\n위 정보를 바탕으로 선택한 영상 모델에 바로 붙여 넣을 수 있는 시각 연출 중심 제작 프롬프트를 작성하세요. 로맨스, 로맨틱 코미디, 감정선, 관계성, 성적 분위기 같은 서사적 해석은 추가하지 말고 사용자가 입력한 장면의 시각적 실행에만 집중하세요. 피사체의 위치·동선·속도·방향·자세 변화, 화면 구도와 여백, 샷 크기·렌즈·초점 심도, 카메라의 이동 경로와 속도, 조명 방향·색온도·명암 대비, 재질·입자·연기·그림자·반사, 공간적 가림과 원근, 컷 길이·전환·시간적 연속성을 구체적인 행동과 촬영 지시로 분해하세요. 각 장면은 시작 상태 → 화면에서 일어나는 변화 → 종료 상태 순서로 쓰고, 추상적인 감정 표현 대신 관찰 가능한 시각 정보와 타임코드를 우선하세요. Hailuo(H3) 프리셋이면 작업 유형에 맞는 참조 라벨·보존 분석·오디오 구역을 포함하고, Seedance 프리셋이면 멀티샷 전개와 타임코드·자산 토큰·편집 범위·보존 목록을 우선하세요. Kling 프리셋으로 원본 편집을 할 때는 SOURCE LOCK과 EDIT ONLY를 프롬프트 앞부분에 두고 전체 재생성을 금지하세요. VFX 덧방일 때는 사용자가 지시한 수정 대상만 바꾸고, 보존 지시를 프롬프트의 앞부분과 품질 제약에 명확히 반복하세요.` },
      ];
      ...
      const response = await invokeLLM({
        ...
        messages: [
          { role: "system", content: "당신은 Google Gemma 4 기반의 시각 연출 디렉터이자 비디오 생성 프롬프트 에디터입니다. 로맨스·로맨틱 코미디·감정선·관계성·성적 분위기를 임의로 추가하지 말고, 사용자가 제공한 장면의 시각적 실행만 구체화합니다. 한국어로 답하며 추상적인 표현보다 피사체의 위치·동선·속도·자세 변화·구도·렌즈·초점·카메라 이동·조명·색감·재질·그림자·반사·가림 관계·컷 전환·시간적 연속성을 관찰 가능한 촬영 지시로 작성합니다. 각 장면을 시작 상태→변화→종료 상태로 구조화하고, 타임코드와 샷 크기를 우선합니다. 참조 이미지 또는 영상 분석이 있으면 보이는 시각적 특징만 활용하며 실제 인물의 신원을 식별하거나 추측하지 않습니다. 노골적인 성행위·성기·나체 중심 묘사·포르노성 지시, 미성년자, 비동의 또는 착취적 성적 상황은 생성하지 말고 중립적인 시각 연출로 바꿉니다. 원본 영상 편집과 VFX 덧방에서는 수정 범위를 엄격히 분리하고, 사용자가 지정하지 않은 인물·카메라·프레이밍·배경을 보존합니다. JSON만 출력합니다." },
          { role: "user", content },
        ],
